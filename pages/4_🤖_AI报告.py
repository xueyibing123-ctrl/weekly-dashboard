...
from database import (
    ...
    get_top_students, get_class_history
)
    ...
    with col_result:
        ...
        if generate_btn:
            ...
            if include_trend and len(history) >= 2:
                ...
                trend_text = f"总分均分：{prev['avg_total']:.1f} → {curr['avg_total']:.1f}，{arrow(delta_total)}；语文{arrow(delta_chinese)}，数学{arrow(delta_math)}，英语{arrow(delta_english)}"
...
语文均分：{avg_chinese:.1f}  数学均分：{avg_math:.1f}  英语均分：{avg_english:.1f}
前五名：{top5_names}
...
"""
                ...
                with st.spinner("AI 正在分析数据，请稍候..."):
                    try:
                        ...
                        st.download_button(
                            ...
                            f"{selected_exam['title']}_AI分析报告.txt",
            ...
            if not api_key:
                ...
                st.download_button(
                    ...
                    f"{selected_exam['title']}_成绩报告.txt",
                    use_container_width=True
                )
...
with tab2:
    ...
    if top_students:
        ...
        award_df = pd.DataFrame([{
            "名次": s["rank"], "姓名": s["name"], "总分": int(s["total"]),
        ...
        st.download_button(
            "⬇️ 导出表彰名单 Excel",
            buf.getvalue(),
            f"{selected_exam['title']}_{award_title}名单.xlsx",
            use_container_width=True
        )
    else:
        st.info("暂无成绩数据")